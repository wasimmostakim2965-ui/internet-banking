...
export default function Auth({onBack,onSuccess,initialMode='signup',demoMode=false}:{onBack:()=>void;onSuccess:()=>void;initialMode?:'signup'|'signin';demoMode?:boolean}) {
  ...
  const [residenceCountry,setResidenceCountry]=useState('Bangladesh');
  ...
  const persistDetails=async()=>{
    ...
    setBusy(true);setError('');
    try{const composed=[firstName.trim(),middleName.trim(),lastName.trim()].filter(Boolean).join(' ');setFullName(composed);await saveProfile(userId,{account_type:'personal',country:residenceCountry,full_name:composed,phone:`${dialCodes[residenceCountry]||''} ${phone.trim()}`.trim(),business_name:null,business_type:null,business_category:null,onboarding_status:'details_pending'});await saveKyc(userId,{legal_name:composed,first_name:firstName.trim(),middle_name:middleName.trim()||null,last_name:lastName.trim(),date_of_birth:dob,place_of_birth:null,country_of_birth:null,secondary_nationality:null,gender,marital_status:maritalStatus,nationality:null,occupation:occupation.trim()||null,employer:employer.trim()||null});await recordAudit(userId,'onboarding.details_saved','profile');go(4)}catch(e){setError((e as Error).message||'We could not save your personal details.')}finally{setBusy(false)}
  };
  ...
  const persistResidence=async()=>{
    ...
    if(!userId||!residenceCountry||!address.trim()||!city.trim()||!postal.trim()){setError('Street address, city and postal code are required.');return}
    setBusy(true);setError('');
    try{await saveKyc(userId,{address_line1:address.trim(),city:city.trim(),region:region.trim(),postal_code:postal.trim(),tax_residence:residenceCountry});await recordAudit(userId,'onboarding.residence_saved','kyc_application');await setupAuthenticator();go(6)}catch(e){setError((e as Error).message||'We could not save your address.')}finally{setBusy(false)}
  };
